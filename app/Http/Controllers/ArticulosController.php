...
class ArticulosController extends Controller
{
    ...
    public function edit($evento_id, $id)
    {
        ...
        if ($articulo->estado === "Recibido") {
            ...
            return redirect()->back()->with('error', 'El artículo ya ha sido calificado o está en revisión');
        }
    ...
    public function destroy(string $id)
    {
        ...
        if ($articulo->estado === "Recibido") {
            ...
            return redirect()->back()->with('error', 'El Articulo ya ha sido evaluado o se encuentra en revisión');
        }
    }
    ...
    public function deleteMultiple(Request $request)
    {
        ...
        if (!empty($ids)) {
            foreach ($ids as $id) {
                ...
                if ($articulo->estado !== "En revisión") {
                    try {
                        articulosAutores::where('articulo_id', $articulo->id)->where('evento_id', $articulo->evento->id)->delete();
    ...
    public function Evaluations($evento_id, $id)
    {
        ...
        $articulos = revisoresArticulos::select('articulo_id', DB::raw('ROUND(AVG(puntuacion), 2) as puntuacion'))
            ...
            ->groupBy('articulo_id') // Agrupar por artículo
            ->get();
