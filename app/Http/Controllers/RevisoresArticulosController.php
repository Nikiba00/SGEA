...
class RevisoresArticulosController extends Controller
{
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($request->has('revisores')) {
            ...
            /*if (!is_array($Revisors)) {
                return back()->withErrors('El formato de los revisores no es válido.');
            }*/
            if (json_last_error() === JSON_ERROR_NONE) {
                ...
                echo "Error al decodificar Datos: " . json_last_error_msg();//CAMBIAR EL ECHO
            }
            return redirect($evento_id . '/revisoresArticulos')->with('info', 'Información Actualizada');
        } else {
            $datos = $request->all();
            // manejo del archivo
            ...
            /////////////////////////////////////////////////////////////////////////////////////////////////////////
            /////////////////////////////////////////////////////////////////////////////////////////////////////////
            // Verificar si los 3 revisores han calificado
            $revisores = revisoresArticulos::where('articulo_id', $id)
                ->where('evento_id', $evento_id)
                ->get();
            ...
            if ($revisores->count() === 3 && $revisores->every(fn($rev) => $rev->puntuacion !== null)) {
                ...
                if (Storage::exists($fileName)) {
                    ...
                    $maxToApprove = $parameters['MaxToApprove'];
                    $minToApprove = $parameters['MinToApprove'];
                } else {
                    return back()->with('error', 'No se encontró el archivo de parámetros.');
                }

                $promedioPuntuacion = $revisores->avg('puntuacion');
                $nuevoEstado = $promedioPuntuacion >= $minToApprove && $promedioPuntuacion <= $maxToApprove ? 'Aceptado' : 'Rechazado';
                \DB::table('articulos')
                    ->where('evento_id', $evento_id)
                    ->where('id', $id)
                    ->update(['estado' => $nuevoEstado]);
            }
            return redirect($evento_id . '/ArticulosPendientes' . '/' . $datos['id_usuario'])->with('info', 'se ha calificado el artículo correctamente');
        }
    }
    ...
    private function NotificarUsuario(usuarios $user, $articuloId)
    {
        ...
        try {
            ...
            if (!$mail->send()) {
                ...
                return redirect()->back()->with('error', 'Error en el envío: ' . $mail->ErrorInfo);
            } else {
    ...
    //MÉTODO PARA ACTUALIZAR EL ESTADO DEL ARTICULO EN BASE A SU CALIFICACIÓN
    /*private function checkAndUpdateArticleStatus(string $articulo_id, string $evento_id)
    {
        // Obtener las revisiones asociadas al artículo
        $revisiones = revisoresArticulos::where('articulo_id', $articulo_id)
            ->where('evento_id', $evento_id)
            ->get(['puntuacion']);

        // Verificar si los 3 revisores han calificado
        if ($revisiones->count() < 3 || $revisiones->contains('puntuacion', null)) {
            return; // Si no hay suficientes revisores o falta alguna puntuación, salir
        }

        // Calcular el promedio de las puntuaciones
        $promedio = $revisiones->avg('puntuacion');

        // Determinar el estado del artículo según el promedio
        $nuevoEstado = $promedio >= 7 ? 'aceptado' : 'rechazado'; // Aprobatoria >= 7

        // Actualizar el estado del artículo
        Articulos::where('id', $articulo_id)
            ->where('evento_id', $evento_id)
            ->update(['estado' => $nuevoEstado]);
    }*/
}