...
<div class="container">
    <div class="search-create">
        ...
        <button class="tooltip" id="create-btn"><i class="las la-plus-circle la-2x"></i><span
                class="tooltip-box">Asignar Revisores</span></button>
    </div>
    @if($articles->isEmpty())
            ...
            <table id="example" class="display nowrap" style="width:100%">
                ...
                <tbody>
                    @foreach ($articles as $ra)
                        <tr>
                            <td><a href="{!! url(session('eventoID') . '/articulo/' . $ra->id) !!}"
                                    style="color:#000;">{!!$ra->titulo!!} </a></td>
                            <td>{!!$ra->estado!!}</td>
                            @foreach ($ra->revisores as $revisor)
                                <td><strong>
                                        <a href="{{ url('usuarios/' . $revisor->usuario->id) }}"
                                            style="color:#000;">{{ $revisor->usuario->nombre_completo }}</a>
                                    </strong></td>
                            @endforeach
                            ...
                            @endfor
                            <td>
                                <a href="{{url(session('eventoID') . '/revisoresArticulo/' . $ra->id . '/edit')}}"><i
                                        class="las la-pencil-alt la-2x"></i></a>
...
<div id="create-modal" class="modal">
    <div class="modal-content">
        ...
        {!! Form::select('revisor-Combo', $usuariosRevisores->pluck('nombre_completo', 'id')->prepend('Seleccionar...',
        ''),
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        //Filtro de los artículos que ya tienen revisores
        const filteredArticles = Object.entries(initialArticles).filter(([id]) => !articlesWithRevisores.includes(parseInt(id)));
        ...
        addBtn.addEventListener('click', () => {
            ...
            if (!selectedValue) {
                Swal.fire({
                    title: 'Advertencia',
                    text: 'Por favor, seleccione un usuario de la lista desplegable.', icon: 'error',
                }); return;
            }
